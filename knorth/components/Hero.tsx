...
import ProjectCarousel from "@/components/designs/ProjectCarousel";
...
const projects = [
  {
    ...
    link: "https://github.com/knorthy",
  },
  {
    title: "Project Two",
    description: "Mobile-first UI design and development using FlutterFlow.",
    tags: ["FlutterFlow", "Design", "Mobile"],
    image: "https://picsum.photos/seed/102/600/400",
    link: "https://github.com/knorthy",
  },
  {
    title: "Project Three",
    description: "Brand identity and visual design crafted in Figma and Illustrator.",
  ...
  {
    title: "Project Four",
    description: "REST API backend built with Spring Boot and Node.js integration.",
    tags: ["SpringBoot", "Node.js", "API"],
    image: "https://picsum.photos/seed/104/600/400",
    link: "https://github.com/knorthy",
  },
  {
    title: "Project Five",
    description: "Content creation pipeline with video editing and motion graphics.",
    tags: ["Premiere Pro", "Canva", "Capcut"],
...
const JOBS = [
  ...
  {
    role: "Freelance Tax Account Associate",
    type: "Remote",
...
function AffiliationList() {
  ...
  return (
    <div className="mt-20">
      <p className="text-xs uppercase tracking-[0.3em] text-foreground/40 mb-10">Leadership &amp; Affiliations</p>

      <div className="relative">

        <div className="relative flex flex-col gap-0">
          {/* Member of — always visible */}
          ...
          {/* Volunteer — shown only when expanded */}
          {showAll && (
          ...
          {!showAll && (
            <div className="relative pl-8">
              <div
                className="absolute -top-24 left-0 right-0 h-32 pointer-events-none z-10"
    ...
    <div className="relative">
      ...
      <div className="relative flex flex-col gap-12">
        {visibleJobs.map((job, i) => (
          ...
          >
            <span
              className="absolute left-0 top-1.5 w-2.5 h-2.5 rounded-full ring-2 ring-background"
            ...
            <ul className="space-y-3">
              {job.bullets.map((b, bi) => (
                <li key={bi} className="flex gap-2 text-sm leading-relaxed" style={{ opacity: 0.75 }}>
        ...
        {/* gradient fade + see more when collapsed */}
        {!showAll && (
          <div className="relative">
            <div
              className="absolute -top-32 left-0 right-0 h-40 pointer-events-none z-10"
              style={{ background: "linear-gradient(to bottom, transparent, var(--background))" }}
            />
            <button
              onClick={() => setShowAll(true)}
              className="relative z-20 mt-2 pl-8 text-sm text-foreground/40 hover:text-foreground/80 transition-colors tracking-[0.2em]"
            >
              see more
            </button>
          </div>
...
export default function Hero() {
  ...
  return (
    <div className="flex flex-col w-full">
      ...
      <section id="home" className="flex flex-col items-center justify-center min-h-screen px-6 max-w-6xl mx-auto w-full">
        <div className="flex flex-col items-center md:items-end text-center md:text-right w-full">
          <div className="relative">
            ...
            <svg
              className="hidden md:block absolute right-full top-1/2 -translate-y-1/2 w-64 h-32 pointer-events-none overflow-visible"
              viewBox="0 0 200 100"
              fill="none"
            >
              <motion.path
          ...
          <div className="relative mb-6">
            <h1 className="text-4xl md:text-6xl font-bold leading-tight">
              ...
              <span className="relative inline-block">
                ...
                <svg
                  className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-[120%] h-[140%] pointer-events-none overflow-visible"
                  viewBox="0 0 100 100"
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {developSkills.map((skill) => (
                    <span
                      key={skill}
                      className="px-3 py-1 rounded-full text-[10px] opacity-80 uppercase tracking-tighter"
                      style={{ border: `1px solid ${ACCENT}40`, color: "var(--foreground)" }}
                    >
                      {skill}
                    </span>
                  ))}
                ...
                <div className="flex flex-wrap gap-2">
                  {createSkills.map((skill) => (
                    <span
                      key={skill}
                      className="px-3 py-1 rounded-full text-[10px] opacity-80 uppercase tracking-tighter"
                      style={{ border: `1px solid ${ACCENT2}40`, color: "var(--foreground)" }}
                    >
                      {skill}
                    </span>
                  ))}
      ...
      <section id="experience" className="min-h-screen flex flex-col justify-center py-24 px-6 max-w-5xl mx-auto w-full">

        {/* heading */}
        <div className="mb-16">
          <p className="text-xs uppercase tracking-[0.3em] text-foreground/40 mb-3">Career</p>
          <h2 className="text-4xl md:text-6xl font-bold">
            Work <span style={{ color: ACCENT3 }}>Experience</span>
          </h2>
        </div>

        {/* WORK EXPERIENCE — full width, collapsible */}
        <ExperienceList />

        {/* LEADERSHIP & AFFILIATIONS — full width below */}
        <AffiliationList />
